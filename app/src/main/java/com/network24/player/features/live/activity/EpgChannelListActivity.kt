...
class EpgChannelListActivity : BaseActivity() {
    ...
    // Double the previous 4.5dp/minute spacing: 540dp per hour.
    ...
    private fun loadChannels() {
        ...
        lifecycleScope.launch {
            try {
                ...
                buildDateSelector(); loadGuideData(); nowHandler.postDelayed(nowLineRunnable, 60_000L)
    ...
    private fun addTimelineHeader(timelineStart: Long, dayStart: Long, dayEnd: Long) {
        ...
        val time = Calendar.getInstance().apply { timeInMillis = if (selectedDay == 0) timelineStart else dayStart }
        val totalMinutes = ((dayEnd - timelineStart) / 60_000L).coerceAtLeast(30L); val slots = (totalMinutes / 30L + 2L).toInt().coerceAtMost(50)
        for (slot in 0 until slots) {
            ...
            timeline.addView(label, LinearLayout.LayoutParams(dp(270), dp(38))); time.add(Calendar.MINUTE, 30)
        }
        ...
        row.addView(timelineFrame); binding.gridContainer.addView(row)
    }
    ...
    private fun addChannelRow(channel: LiveChannel, index: Int, timelineStart: Long, dayEnd: Long) {
        ...
        programs.forEach { program ->
            val start = (program.startTimestamp ?: cursor).coerceIn(timelineStart, dayEnd); val stop = (program.stopTimestamp ?: (start + 30L * 60L * 1000L)).coerceIn(timelineStart, dayEnd)
        ...
        timelineFrame.addView(timeline, FrameLayout.LayoutParams(-2, dp(64))); if (selectedDay == 0) addNowLine(timelineFrame, timelineStart, 64)
        row.addView(timelineFrame); binding.gridContainer.addView(row)
    }

    private fun addEmptyBlock(parent: LinearLayout, durationMs: Long) { val minutes = (durationMs / 60_000L).coerceAtLeast(5L); parent.addView(View(this), LinearLayout.LayoutParams((minutes * minuteWidthDp).toInt().coerceAtLeast(dp(18)), dp(64))) }

    private fun addProgramBlock(parent: LinearLayout, channel: LiveChannel, program: EpgEntity, durationMs: Long) {
        val now = System.currentTimeMillis(); val start = program.startTimestamp ?: Long.MAX_VALUE; val stop = program.stopTimestamp ?: Long.MIN_VALUE; val isNow = start <= now && stop > now
        val title = program.title?.takeIf { it.isNotBlank() } ?: "No Program Info"; val minutes = (durationMs / 60_000L).coerceAtLeast(5L); val cardWidth = (minutes * minuteWidthDp).toInt().coerceAtLeast(dp(55))
        val card = FrameLayout(this).apply {
            isFocusable = true; isClickable = true; background = programBackground(channel, program, isNow, false)
        ...
        val text = TextView(this).apply { this.text = if (isNow) "NOW • $title" else title; gravity = Gravity.CENTER_VERTICAL; setTextColor(Color.WHITE); textSize = if (isNow) 15f else 14f; setPadding(dp(10), dp(4), dp(10), dp(7)); maxLines = 1; isSingleLine = true; ellipsize = android.text.TextUtils.TruncateAt.END }
        card.addView(text, FrameLayout.LayoutParams(-1, -1))
        if (isNow && stop > start) { val progress = ((now - start).toFloat() / (stop - start).toFloat()).coerceIn(0f, 1f); val progressWidth = (cardWidth * progress).toInt().coerceAtLeast(dp(3)); val progressLine = View(this).apply { setBackgroundColor(Color.WHITE) }; card.addView(progressLine, FrameLayout.LayoutParams(progressWidth, dp(3), Gravity.BOTTOM or Gravity.START)) }
    ...
    private fun addNowLine(parent: FrameLayout, timelineStart: Long, heightDp: Int) {
        val now = System.currentTimeMillis(); val minutes = ((now - timelineStart).coerceAtLeast(0L) / 60_000L).toFloat(); val left = (minutes * minuteWidthDp).toInt(); val line = View(this).apply { setBackgroundColor(Color.rgb(255, 152, 0)) }; val params = FrameLayout.LayoutParams(dp(2), dp(heightDp), Gravity.TOP or Gravity.START); params.leftMargin = left; parent.addView(line, params)
    }
